...
void LevelCompactionBuilder::SetupOtherFilesWithRoundRobinExpansion() {
  ...
  for (size_t i = start_index + 1; i < level_files.size(); i++) {
    ...
    if (!compaction_picker_->ExpandInputsToCleanCut(cf_name_, vstorage_,
      ...
      // EnumerateLog::Log("Early stop in RoundRobin trivial move for constraint 1a-1: " + std::to_string(start_level_inputs_.size()));
      return;
    }
    ...
    if (!output_level_inputs.empty() &&
      ...
      // Constraint 1a
      tmp_start_level_inputs.clear();
      // EnumerateLog::Log("Early stop in RoundRobin trivial move for constraint 1a-2: " + std::to_string(start_level_inputs_.size()));
      return;
    }
    ...
    if (curr_bytes_to_compact > mutable_cf_options_.max_compaction_bytes) {
      // Constraint 2
      tmp_start_level_inputs.clear();
      // EnumerateLog::Log("Early stop in RoundRobin trivial move for constraint 2: " + std::to_string(start_level_inputs_.size()));
      return;
    }
    ...
    if (start_lvl_curr_bytes_to_compact > start_lvl_max_bytes_to_compact) {
      // EnumerateLog::Log("Early stop in RoundRobin trivial move for constraint 3 (" + std::to_string(start_lvl_curr_bytes_to_compact) + ")");
      return;
    }
  }
}
...
bool LevelCompactionBuilder::SetupOtherInputsIfNeeded() {
  ...
  if (output_level_ != 0) {
    // EnumerateLog::Log("File number in before SetupOtherInputsIfNeeded(): " + std::to_string(start_level_inputs_.size()));
    ...
    // EnumerateLog::Log("File number after SetupOtherFilesWithRoundRobinExpansion(): " + std::to_string(start_level_inputs_.size()));
    ...
    // EnumerateLog::Log("File number after SetupOtherInputs(): " + std::to_string(start_level_inputs_.size()));
...
Compaction* LevelCompactionBuilder::PickCompaction() {
  ...
  // to a clean cut.
  SetupInitialFiles();
  // EnumerateLog::Log("File number in after SetupInitialFiles(): " + std::to_string(start_level_inputs_.size()));
  if (start_level_inputs_.empty()) {
    return nullptr;
  }
...
bool LevelCompactionBuilder::TryExtendNonL0TrivialMove(int start_index,
  ...
  if (start_level_inputs_.size() == 1 &&
    ...
    // EnumerateLog::Log("File number in before TryExtendNonL0TrivialMove(): " + std::to_string(start_level_inputs_.size()));
    const std::vector<FileMetaData*>& level_files =
        vstorage_->LevelFiles(start_level_);
...
bool LevelCompactionBuilder::PickFileToCompact() {
  ...
  // WEI RAN
  // if the start_level is L1 and output_level is L2, we use
  // our AllFilesEnumerator to put an unselected file to the
  // first place
  vstorage_->PickUnselectedFile(start_level_, output_level_, ioptions_, mutable_cf_options_);
  ...
  for (cmp_idx = vstorage_->NextCompactionIndex(start_level_);
    ...
    vstorage_->GetOverlappingInputs(output_level_, &smallest, &largest,
                                    &output_level_inputs.files);
    if (output_level_inputs.empty()) {
      if (start_level_ > 0 && !CS561Option::skip_extend_non_l0_trivial_move &&
